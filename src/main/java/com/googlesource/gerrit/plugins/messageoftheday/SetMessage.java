...
public class SetMessage implements RestModifyView<ConfigResource, MessageInput> {
  ...
  public Response<?> apply(ConfigResource resource, MessageInput input)
    ...
    try {
      ...
      throw new UnprocessableEntityException("plugin cfg not found or could not be loaded", e);
    }

    String htmlFileId = cfg.getString(SECTION_MESSAGE, null, KEY_ID);
    if (Strings.isNullOrEmpty(htmlFileId)) {
      logger.atInfo().log(
          "'id' is not configured in the plugin configuration. Choosing a default id.");
      htmlFileId = "default";
    }
    ...
    if (input.expireAfter != null) {
      long ms =
          MILLISECONDS.convert(ConfigUtil.getTimeUnit(input.expireAfter, 0, SECONDS), SECONDS);
      ...
      cfg.setString(SECTION_MESSAGE, null, KEY_ID, htmlFileId);
    }
